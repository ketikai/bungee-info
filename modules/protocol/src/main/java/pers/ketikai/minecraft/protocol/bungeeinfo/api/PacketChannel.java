...
package pers.ketikai.minecraft.protocol.bungeeinfo.api;
...
public abstract class PacketChannel implements PacketChannelAdapter {
    ...
    private static final class SimplePacketChannel extends PacketChannel {
        ...
        private static final Logger log = Logger.getLogger(SimplePacketChannel.class.getName());
        ...
        public final void send(short id, @NotNull Object payload, @NotNull Entry<String, Object>... context) throws PacketException {
            ...
            if (codec == null) {
                log.fine(String.format("No codec found for id %s", String.valueOf(id)));
                return;
            }
            log.fine(String.format("Sending packet %s with id %s", payload, String.valueOf(id)));
        ...
        public final void receive(byte @NotNull [] packet, @NotNull Entry<String, Object>... context) throws PacketException {
            ...
            PacketCodec codec = codecs.get(id);
            if (codec == null) {
                log.fine(String.format("No codec found for id %s", id));
                return;
            }
            byte[] payload = new byte[wrapped.remaining()];
            wrapped.get(payload);
            log.fine(String.format("Receiving packet %s with id %s", Arrays.toString(payload), String.valueOf(id)));
